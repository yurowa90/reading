...
test.describe("Phase 1 흐름", () => {
  ...
  //  Phase 3:
  //   - 교사 평가 기간 설정 → 학생 좋아요/별점 → 기간 중 집계 비공개 확인
  //   - 자기 작품 좋아요/별점 불가 확인
  //   - 댓글/답글 등록·수정·삭제, 연속 등록 제한
  //   - 댓글 신고 → 교사 신고 큐에서 숨김/처리
  //  Phase 4:
  //   - 평가 종료 후 교사 후보 보드에서 상위 20% 후보 표시 확인
  //   - 교사 루브릭 저장 → 최종 우수작 선정 → 갤러리 ★우수작 배지 노출
  //   - 학생은 후보 보드/루브릭에 접근 불가 확인
  //  Phase 5:
  //   - 산파법 대화 시작 → 질문 하나 표시 → 답변 → 다음 단계 질문
  //   - 마무리 후 "서평 쓰러 가기" 링크(대필 없음) 확인
  //   - (AI_API_KEY 설정 시) Gemini 응답이 스키마 검증을 통과하는지 확인
  //   - 다른 학생 대화 세션 접근 불가 확인
});
