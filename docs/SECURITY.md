...
## AI 챗봇 (Phase 5, 구현됨) — `lib/ai/`, `0008_chat.sql`

- **키 보관**: `AI_API_KEY`는 서버 환경 변수에만. `NEXT_PUBLIC_` 접두사를 쓰지 않으며
  `provider.ts`는 `import "server-only"`로 클라이언트 번들 유입을 차단한다. 키를 폼으로 입력받지 않는다.
- **개인정보 미전송**: AI에는 책 제목·저자·학생 답변·수집 문장(따옴표)만 보낸다.
  학생 이름·이메일·학번·프로필은 전송하지 않는다. 화면에서도 개인정보 입력을 하지 말라고 안내한다.
- **대필 차단**: 응답은 자유 텍스트가 아니라 `{stage, question, hint?, nextStage}` 구조만 받고
  Zod로 검증한다. 완성 서평/문단을 반환하지 않는다. 시스템 프롬프트에도 대필 금지를 명시.
- **진행 통제**: 학생이 답해야 다음 질문을 생성한다. 단계 순서는 서버가 강제(모델 값 무시).
  스키마 위반·호출 실패 시 안전한 mock 질문으로 폴백한다.
- **RLS**: `chat_sessions`/`chat_messages`는 본인만 CRUD, 담당 교사는 열람(오남용 점검)만.
- **로깅**: 키·토큰 등 민감정보를 로그에 남기지 않는다(오류 메시지만 기록).
