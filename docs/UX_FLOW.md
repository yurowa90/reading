...
## 독서 산파법 대화 흐름 (Phase 5)

```
학생: 도서 선택 → 대화 시작 → (관찰→해석→근거→반론→연결→정리→마무리)
   각 단계에서 질문 하나 → 학생이 답 → 다음 질문 → 마무리 후 "서평 쓰러 가기"
```

- AI는 한 번에 질문 하나만 하고, 학생이 답해야 다음 단계로 넘어간다.
- AI는 서평을 대신 쓰지 않는다(마무리 단계도 학생이 직접 요약).
- 개인정보 입력 금지 안내를 상단에 표시한다. 키 미설정 시 예시(mock) 질문으로 동작.
- 담당 교사는 학생 대화를 열람만 할 수 있다(오남용 점검).
