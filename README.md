...
- **Phase 5**: 독서 산파법 챗봇(7단계·한 번에 질문 하나·대필 차단) · **Google Gemini 연동**(mock 폴백).

- **Phase 6(진행)**: 학생 독서 포트폴리오·교사 대시보드/통계·인쇄(PDF) 내보내기.

> **검증 상태**: 마이그레이션(0001~0008)과 핵심 RLS 시나리오를 로컬 Postgres에서 실행해
> 통과했습니다(`bash supabase/tests/validate_local.sh`). Gemini 실호출도 확인했습니다.
> Storage 실버킷 런타임만 실제 Supabase에서 확인이 필요합니다(정책 SQL은 적용 검증됨).

## AI 챗봇(Gemini) 설정

`.env.local`(또는 배포 환경 변수)에 **본인의 Google Gemini API 키**를 넣으면 실제 AI로 동작합니다.

```
AI_PROVIDER=gemini
AI_API_KEY=<https://aistudio.google.com/apikey 에서 발급한 키>
AI_MODEL=gemini-2.0-flash   # 선택(기본값)
```

- 키는 **서버 환경 변수에만** 둡니다. `NEXT_PUBLIC_`을 붙이지 마세요(브라우저 노출 금지).
- 키가 없으면 안전한 예시(mock) 질문으로 동작하므로 앱은 그대로 작동합니다.
- AI에는 책 제목·저자·학생 답변·수집 문장만 전달되며, 학생 이름·이메일·학번은 전송하지 않습니다.
...
2. 마이그레이션을 순서대로 적용합니다. SQL Editor에 붙여넣거나 Supabase CLI를 사용합니다.
   ...
   supabase/migrations/0008_chat.sql      # Phase 5: 산파법 챗봇 세션/메시지
   ```
