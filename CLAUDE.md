...
**Phase 4 완료.** Phase 1(인증/학급/도서/문장카드) + Phase 2(서평·북포스터·게시 승인·갤러리)
+ Phase 3(댓글·좋아요·별점·상호평가 기간) + Phase 4(우수작 후보 점수·교사 루브릭·최종 선정)까지 구현.
...
- 주의: Storage·RLS 런타임은 로컬 Supabase 미구성으로 미검증(코드/정책/절차만).

Phase 5 이후(독서 산파법 AI 챗봇), Phase 6(포트폴리오·대시보드·통계)는 문서 설계만 존재.

## 절대 하면 안 되는 작업

- Phase 5 이후 기능을 미리 구현하거나 작동하지 않는 버튼/빈 화면 만들기
...
`docs/ROADMAP.md`의 Phase 5(독서 산파법 챗봇) 항목을 따른다. AI는 서평을 대필하지 않고 한 번에
질문 하나만 제시하며, 응답은 Zod로 구조 검증하고 제공자 교체용 adapter 구조를 쓴다. 학생 식별정보를
AI API로 보내지 않는다. 새 테이블은 `supabase/migrations/`에 번호를 이어 추가하고 RLS·거부 테스트를 동반한다.
