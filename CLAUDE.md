...
**Phase 3 완료.** Phase 1(인증/학급/도서/문장카드) + Phase 2(서평·북포스터·게시 승인·갤러리)
+ Phase 3(댓글·좋아요·별점·상호평가 기간)까지 구현.
...
Phase 4 이후(우수작 후보 점수·교사 루브릭·AI 챗봇)는 문서 설계만 존재.

## 절대 하면 안 되는 작업

- Phase 4 이후 기능을 미리 구현하거나 작동하지 않는 버튼/빈 화면 만들기
...
`docs/ROADMAP.md`의 Phase 4(우수작 후보 점수·교사 루브릭·최종 선정) 항목을 따른다. 새 테이블은
`supabase/migrations/`에 번호를 이어 추가하고, 반드시 RLS 정책과 거부 테스트를 함께 작성한다.
우수작 후보 점수 계산식(베이지안 보정·정규화)은 `docs/DATA_MODEL.md` 참조.
